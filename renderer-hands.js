...
export function drawRedBoxer() {
    ...
    // BEZWZGLĘDNY RESET: Czyścimy wszelkie krzywe obroty, które mogły narzucić inne pliki w pętli gry
    ctx.setTransform(1, 0, 0, 1, 0, 0);

    const bounceOffset = boxerRed.isChargingSuper ? 0 : Math.sin(boxerRed.animTimer) * 4;
    
    // Obliczanie absolutnego kąta patrzenia na współrzędne X i Y niebieskiego
    ...
    // Matryca przesunięcia i czystego obrotu twarzą do rywala
    ctx.save(); 
    ...
    if (boxerRed.isChargingSuper && !boxerRed.isPunching) {
        leftY = -boxerRed.radius - 2; rightY = -boxerRed.radius - 2;
    } 
